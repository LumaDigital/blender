...
bend_bones_spine_only = ['c_waist_bend.x', 'c_root_bend.x', 'c_spine_01_bend.x', 'c_spine_02_bend.x', 'c_bot_bend', 'c_neck_01.x']
...
class ARP_OT_GE_check_rig(Operator):
    ...
    def execute(self, context):
        ...
        if len(self.invalid_modifiers) > 0:
            self.message_final += '\n---------- Invalid Modifiers ----------'
        ...
        if len(self.scale_object_action) > 0:
            self.message_final += '\n-------- Scaled Object --------'
        ...
        if len(self.preserve_volume_enabled) > 0:
            self.message_final += '\n-------- Preserve Volume ---------'
        ...
        if len(self.stretch_controllers) > 0:
            self.message_final += '\n------------ Stretch -------------'
        ...
        if len(self.auto_stretch_controllers) > 0:
            self.message_final += '\n---------- Auto-Stretch ----------'
...
class ARP_OT_set_mped_rig(Operator):
    ...
    def execute(self, context):
        ...
        if arm.proxy or arm.override_library:
        ...
        try:
            ...
            if bpy.context.active_object.type != "ARMATURE":
                ...
                if bpy.context.active_object.override_library:
                    ...
                    if utils.blender_version._float >= 290 and utils.blender_version._float < 291:
            ...
            self.arm_scale = get_object(self.armature_name+'_arpexport').scale[0]
                ...
                if not scn.arp_ue_root_motion:
                    if len(root_armature.constraints):
                        if root_armature.constraints[0].type == "COPY_TRANSFORMS":
                            ...
                            cns1.target = get_object(self.armature_name + "_arpexport")
                            cns2 = root_armature.constraints.new("COPY_SCALE")
                            cns2.name = "Copy Scale"
                            cns2.target = get_object(self.armature_name + "_arpexport")

                if utils.blender_version._float < 290:# 2.90 version and higher doesn't need it
            ...
            if scn.arp_bake_type == 'ACTIONS' and scn.arp_export_separate_fbx:
        ...
        #   pass
            ...
            for obj in bpy.data.objects:
                if "_arpbaseobject" in obj.name:
                    if get_object(obj.name.replace("_arpbaseobject", "")):
                        #rename final objects to "_arpexport"
                        obj_name = obj.name.replace("_arpbaseobject", "")
                        get_object(obj_name).name = obj_name + "_arpexport"
            ...
            if scn.arp_keep_bend_bones and scn.arp_push_bend and (get_object(self.armature_name).arp_secondary_type == "ADDITIVE" or get_object(self.armature_name).arp_secondary_type == "TWIST_BASED"):
                print("\nRevert Push Bend Bones")
            ...
            for obj in bpy.data.objects:
                ...
                if "_arpexport" in obj.name or obj.name == "rig_humanoid" or obj.name == scn.arp_export_rig_name or obj.name == "rig_mped" or "arp_dummy_mesh" in obj.name:
...
def create_copies(self):
    ...
    if self.armature_proxy_name or self.armature_source_name:
        ...
        # unprotect layers
        for i in range(0,32):
            rig_arpexport.data.layers_protected[i] = False

        # hack for driver updates
        drivers_arm = rig_arpexport.animation_data.drivers
    ...
    for obj_name in self.char_objects + self.spline_objects:
        ...
        if ob.type == "MESH" or ob.type == "CURVE":
            ...
            ob.parent = get_object(self.armature_name+"_arpexport")
    ...
    # retarget modifiers
    # assign the _arpexport rig as target for armature and hooks modifiers
    # and retarget target object
    for obj in bpy.data.objects:
        if not "_arpexport" in obj.name:
            continue
        ...
        for mod in obj.modifiers:
            ...
            if 'target' in dir(mod):
                if mod.target:
                    mod.target = get_object(mod.target.name + '_arpexport')
            ...
            if mod.object != None:
                if mod.object.name == deforming_arm:
                    mod.object = get_object(self.armature_name + '_arpexport')
    ...
    if len(self.spline_objects):
        ...
        for b in rig_arpexport.pose.bones:
            ...
            if len(b.constraints):
                for cns in b.constraints:
                    ...
                    if cns.target == None:
                        continue
                    if not "_arpexport" in cns.target.name:
                        cns.target = get_object(cns.target.name+"_arpexport")
...
def init_armatures_scales(armature_name, armature_add_name, self):
    ...
    if armature_add_name:
        rig_add = get_object(armature_add_name + '_arpexport')
    if rig_add:
    ...
    # Warning, reset Child Of constraints after Init Scale is for Blender versions < 2.81a only
    """
    print("RESET CHILD OF")
    if blender_version_float <= 281.16 and blender_version_char == "":
        print("Blender version:", blender_version_float, ", Child Of constraints must be reset")
        # Reset set inverse child of constraints
        for pbone in bpy.context.active_object.pose.bones:
            # do not reset child of of custom bones... issues
            if not is_custom_bone(pbone.name) and len(pbone.constraints) > 0:
...
def parent_meshes_to_baked_armature(self, baked_armature_name):
    ...
    armature_arpexport = get_object(self.armature_name + "_arpexport")

    # set to rest pose
    armature_base.data.pose_position = 'REST'
    armature_arpexport.data.pose_position = 'REST'
    ...
    if armature_arpexport.animation_data:
        armature_arpexport.animation_data.action = None

    # init location, rotation
    armature_arpexport.location = armature_arpexport.rotation_euler = [0,0,0]
    ...
    armature_arpexport.data.pose_position = 'POSE'
...
def _set_mannequin_orientations(self):
    ...
    for i in bpy.data.objects:
        if not i.name.endswith("_arpexport"):
            continue
...
def _bake_pose(baked_armature_name):
    ...
    for pbone in bpy.context.active_object.pose.bones:
        mat_list[pbone.name] = pbone.matrix.copy()

    _constraint_rig(True)

    for bname in mat_list:
        pbone = get_pose_bone(bname)
        pbone.matrix = mat_list[bname]
    """
...
def _bake_all(armature_name, baked_armature_name, self):
    ...
    def prepare_for_baking():
        ...
        # when exporting root motion, initialize the baked armature location to the arp_armature location
...
def get_char_objects(arm_name=None, arm_add_name=None, arm_proxy=None, arm_source=None):
    ...
    for obj in objs:
        ...
        elif obj.type == "CURVE" and obj.name.startswith("spline_ik_curve"):# Spline IKs curves
...
def _set_units_x100_baked(armature_name, self):
    ...
    if unit_system.system == 'NONE' or (unit_system.system != 'NONE' and (unit_system.scale_length > 1.0-0.0003 and unit_system.scale_length < 1.0+0.0003)):
        ...
        rig = get_object(armature_name+'_arpexport')
...
def _set_humanoid_rig(armature_name, armature_add_name, manual_set_debug, self):
    ...
    for obj in bpy.context.view_layer.objects:
        if not(obj.type == 'MESH' and ("_arpexport" in obj.name or manual_set_debug)):
            continue
    ...
    if arp_armature.arp_secondary_type == "TWIST_BASED":
        for side in [".l", ".r"]:
            for bname in ['arm', 'forearm', 'thigh', 'leg']:
                ...
                c_stretch_dict[c_stretch_name] = {"head":c_stretch.head.copy(), "tail":c_stretch.tail.copy(), "roll":c_stretch.roll, "parent":b_parent_n, "softlink":sftlink}
            ...
            for side in [".l", ".r"]:
                ...
                if bone_name == "thigh_twist" or bone_name == "leg_twist":
                    ...
                    if bone_name == "leg_twist":
                        ...
                        twist_bones_dict[twist_bone_name] = {"head":twist_bone.head.copy(), "tail":twist_bone.tail.copy(), "roll":twist_bone.roll, "parent":b_parent, "softlink":is_softlink_bone(twist_bone_name), 'type':'leg_main'}
                        ...
                        if twist_bones_amount > 1:
                            for t_idx in range(2, twist_bones_amount+1):
                                ...
                                b_parent = 'leg_stretch'+side
                                twist_bones_dict[leg_twist_idx_name] = {"head":leg_twist_idx.head.copy(), "tail":leg_twist_idx.tail.copy(), "roll":leg_twist_idx.roll, "parent":b_parent, "softlink":is_softlink_bone(leg_twist_idx_name), 'type':'leg_multi'}

                    if bone_name == "thigh_twist":
                        # first thigh twist bone (default)
                        b_parent = "thigh_stretch"+side
                        twist_bones_dict[twist_bone_name] = {"head":twist_bone.head.copy(), "tail":twist_bone.tail.copy(), "roll":twist_bone.roll, "parent":b_parent, "softlink":is_softlink_bone(twist_bone_name), 'type':'thigh_main'}
                        # multiple leg twist bones
                        if twist_bones_amount > 1:
                            for t_idx in range(2, twist_bones_amount+1):
                                ...
                                b_parent = 'thigh_stretch'+side
                                twist_bones_dict[thigh_twist_idx_name] = {"head":thigh_twist_idx.head.copy(), "tail":thigh_twist_idx.tail.copy(), "roll":thigh_twist_idx.roll, "parent":b_parent, "softlink":is_softlink_bone(thigh_twist_idx_name), 'type':'thigh_multi'}
                ...
                if bone_name == "c_arm_twist_offset" or bone_name == "forearm_twist":
                    ...
                    if bone_name == "forearm_twist":
                        ...
                        twist_bones_dict[twist_bone_name] = {"head":twist_bone.head.copy(), "tail":twist_bone.tail.copy(), "roll":twist_bone.roll, "parent":b_parent, "softlink":is_softlink_bone(twist_bone_name), 'type':'forearm_main'}
                        ...
                        if twist_bones_amount > 1:
                            for t_idx in range(2, twist_bones_amount+1):
                                ...
                                b_parent = 'forearm_stretch'+side
                                twist_bones_dict[forearm_twist_idx_name] = {"head":forearm_twist_idx.head.copy(), "tail":forearm_twist_idx.tail.copy(), "roll":forearm_twist_idx.roll, "parent":b_parent, "softlink":is_softlink_bone(forearm_twist_idx_name), 'type':'forearm_multi'}

                    if bone_name == "c_arm_twist_offset":
                        # first arm twist bone (default)
                        b_parent = "arm_stretch"+side
                        twist_bones_dict[twist_bone_name] = {"head":twist_bone.head.copy(), "tail":twist_bone.tail.copy(), "roll":twist_bone.roll, "parent":b_parent, "softlink":is_softlink_bone(twist_bone_name), 'type':'arm_main'}
                        # multiple leg twist bones
                        if twist_bones_amount > 1:
                            for t_idx in range(2, twist_bones_amount+1):
                                ...
                                b_parent = 'arm_stretch'+side
                                twist_bones_dict[arm_twist_idx.name] = {"head":arm_twist_idx.head.copy(), "tail":arm_twist_idx.tail.copy(), "roll":arm_twist_idx.roll, "parent":b_parent, "softlink":is_softlink_bone(arm_twist_idx_name), 'type':'arm_multi'}
    ...
    for dupli in limb_sides.ear_sides:
        for ear_id in range(0, 17):
            ...
            if is_deforming(c_bone.name):
                bones_dict[c_bone.name] = {"head":c_bone.head.copy(), "tail":c_bone.tail.copy(), "roll":c_bone.roll, "deform":True, "softlink":bone_softlink}
    ...
    for bname in bones_dict:
        ...
        bones_dict[bname] = {"head": edit_bone.head.copy(), "tail":edit_bone.tail.copy(), "roll":edit_bone.roll, "deform":b_use_deform, "x_axis":edit_bone.x_axis.copy(), "softlink":bone_softlink}
    ...
    if is_facial_enabled(arp_armature):
        ...
        if jaw:
            if 'c_jawbone.x' in bones_dict:
                bones_dict['c_jawbone.x'] = {"head":jaw.head.copy(), "tail":jaw.tail.copy(), "roll":jaw.roll, "deform":bones_dict['c_jawbone.x']["deform"], "softlink":jaw_softlink}
        ...
        for side in sides:
            ...
            for pos in ["_top", "_bot"]:
                ...
                bones_dict[bname] = {"head":eyel.head.copy(), "tail":eyel.tail.copy(), "roll":eyel.roll, "deform":bones_dict[bname]["deform"], "softlink":eyelid_softlink}
    ...
    if neck_ref:
        ...
        if neck_count > 1:
            for idx in range(1, neck_count):
                ...
                subneck_coords[subneck_cont_name] = {"head":subneck_deform.head.copy(), "tail":subneck_deform.tail.copy(), "roll":subneck_deform.roll, "parent":subneck_parent_name, "softlink":subneck_softlink}
    ...
    for side in limb_sides.wing_sides:
        ...
        for bname in arm_feathers+forearm_feathers+hand_feathers:
            ...
            wings_coords[bname] = {"head":ebone.head.copy(), "tail":ebone.tail.copy(), "roll":ebone.roll, "parent":b_parent, "softlink":feather_softlink}
    ...
    for pbone in humanoid_armature.pose.bones:
        ...
        if is_custom_bone(pbone.name) or is_bend_bone(pbone.name) or (pbone.name[:-2] in additional_facial_bones) or pbone.name.startswith('c_toes_') or pbone.name.startswith("c_tail_") or pbone.name.startswith("c_ear_") or pbone.name.startswith("c_breast_"):
            if len(pbone.constraints) != 0:
                continue
    ...
    for obj_name in collected_meshes:
        ...
        for modif in obj.modifiers:
            ...
            if obj_name != 'arp_dummy_mesh_arpexport' and modif.object.name != arp_armature.name:
                continue
...
def spline_ik_export_to_dict(name, side, dict):
    ...
    for i in range(1, amount+1):
        ...
        dict[bname] = {"head":ebone.head.copy(), "tail":ebone.tail.copy(), "roll":ebone.roll, "parent":b_parent, "type":"ik_spline", "arp_bone":ebone.name, "softlink":bone_softlink}
                ...
                if is_deforming(bone_name + side):
                    ...
                    if scn.arp_export_twist or arp_armature.arp_secondary_type == "TWIST_BASED":
                        ...
                        if "leg_twist" in bone_name:
                            # first leg twist bone (default)
                            b_parent = "leg_stretch"+side
                            bones_coords["leg_twist"+side] = {"head":ebone.head.copy(), "tail":ebone.tail.copy(), "roll":ebone.roll, "parent":b_parent, "arp_bone":ebone.name, "softlink":is_softlink_bone(ebone.name)}

                            # multiple leg twist bones
                            if twist_bones_amount > 1:
                                for t_idx in range(2, twist_bones_amount+1):
                                    leg_twist_idx = get_edit_bone('leg_twist_'+str(t_idx)+side)
                                    b_parent = 'leg_stretch'+side
                        ...
                        if "thigh_twist" in bone_name:
                            # first thigh twist bone (default)
                            b_parent = "thigh_stretch"+side
                            bones_coords["thigh_twist"+side] = {"head":ebone.head.copy(), "tail":ebone.tail.copy(), "roll":ebone.roll, "parent":b_parent, "arp_bone":ebone.name, "softlink":is_softlink_bone(ebone.name)}
                            # multiple thigh twist bones
                            if twist_bones_amount > 1:
                                for t_idx in range(2, twist_bones_amount+1):
                                    thigh_twist_idx = get_edit_bone('thigh_twist_'+str(t_idx)+side)
                                    b_parent = 'thigh_stretch'+side
                    ...
                    if bone_name == "c_thigh_b":
                        ...
                        if c_thigh_b.parent:
                            # in case of 3 leg bones, get the parent of the parent (controller < ik chain < root)
                ...
                if is_deforming(bone_name + side):
                    ...
                    if scn.arp_export_twist or arp_armature.arp_secondary_type == "TWIST_BASED":
                        ...
                        if "c_arm_twist" in bone_name:
                            # first arm twist bone (default)
                            b_parent = "arm_stretch" + side
                            bones_coords["arm_twist" + side] = {"head":ebone.head.copy(), "tail":ebone.tail.copy(), "roll":ebone.roll, "parent":b_parent, "arp_bone":ebone.name, "softlink":is_softlink_bone(ebone.name)}
                            # multiple arm twist bones
                            if twist_bones_amount > 1:
                                for t_idx in range(2, twist_bones_amount+1):
                                    arm_twist_idx = get_edit_bone('arm_twist_'+str(t_idx)+side)
                                    b_parent = 'arm_stretch'+side
                        ...
                        if "forearm_twist" in bone_name:
                            # first forearm twist bone (default)
                            b_parent = "forearm_stretch" + side
                            bones_coords["forearm_twist" + side] = {"head":ebone.head.copy(), "tail":ebone.tail.copy(), "roll":ebone.roll, "parent":b_parent, "arp_bone":ebone.name, "softlink":is_softlink_bone(ebone.name)}
                            # multiple forearm twist bones
                            if twist_bones_amount > 1:
                                for t_idx in range(2, twist_bones_amount+1):
                                    forearm_twist_idx = get_edit_bone('forearm_twist_'+str(t_idx)+side)
                                    b_parent = 'forearm_stretch'+side
            ...
            if self.is_arp_armature:
                ...
                if "thigh_stretch" in bone_name:
                    if not bone_name.startswith("c_") and not bone_softlink:# if softlink, we need to keep the actual thigh stretch position
                        ...
                        cns_loc.target = arp_armature
                        cns_loc.subtarget = "thigh_twist" + bone_side
                    #else:# twist based controller
                    #    add_copy_transf(pbone, tar=arp_armature, subtar=bone_name, no_scale=bone_softlink)
    ...
    def draw(self, context):
        ...
        if scn.arp_export_rig_type == 'humanoid':
            if scn.arp_export_show_panels == "RIG":
                ...
                if scn.arp_engine_type == "unreal":
                    ...
                    row.prop(scn, "arp_ue_root_motion")
        ...
        if scn.arp_export_rig_type == 'mped':
            if scn.arp_export_show_panels == "RIG":
                ...
                if scn.arp_engine_type == "unreal" and self.is_arp_armature:
                    box = layout.box()
                    box.label(text="Unreal Options:")
                    col = box.column(align=True)
                    row = col.row(align=True)
...
def update_arp_tab():
    try:
        ...
        pass
    ARP_PT_auto_rig_GE_panel.bl_category = bpy.context.preferences.addons[__package__].preferences.arp_tab_name
...
def register():
    ...
    bpy.types.Scene.arp_ue_root_motion = BoolProperty(name='Root Motion', description='The "c_traj" controller animation will be baked to the root node for root motion in Unreal', default=False)#, update=update_root_motion_btn)
