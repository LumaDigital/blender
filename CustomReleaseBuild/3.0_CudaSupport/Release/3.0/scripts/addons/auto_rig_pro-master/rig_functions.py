...
class ARP_MT_layers_sets_menu(Menu):
    ...
    def draw(self, _context):
        ...
        layout.popover(panel='ARP_PT_layers_sets_edit', text="Edit Layer...")    
...
class ARP_OT_layers_set_import_preset(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        try:         
            # custom presets       
            custom_dir = bpy.context.preferences.addons[__package__].preferences.rig_layers_path
...
class ARP_OT_layers_sets_remove_bones(Operator):
    ...
    bl_label = "Removes All Bones In Set"   
...
def _set_picker_camera(self):
    ...
    if rig.proxy_collection:# proxy support
    ...
    def draw(self, context):
       ...
       # Leg
        ...
        if is_selected('c_jawbone' + bone_side, selected_bone_name):
            if len(get_pose_bone('c_jawbone' + bone_side).keys()) > 0:
...
def update_arp_tab():
    ...
    ArpRigToolsPanel.bl_category = bpy.context.preferences.addons[__package__].preferences.arp_tools_tab_name
...
def update_layers_set_presets():
    presets_directory = bpy.context.preferences.addons[__package__].preferences.rig_layers_path
