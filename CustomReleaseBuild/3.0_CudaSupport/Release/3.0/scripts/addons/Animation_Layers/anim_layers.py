    ...
    for obj in bpy.context.scene.objects:
        ...
        if obj.als.turn_on:
            ...
            if obj.als.layer_index > len(obj.Anim_Layers) - 1:
                ...
                #calling index function to make sure the relevant action active
...
def draw_override_error(self, context):
    obj = context.object
    self.layout.label(text = obj.name + " is an override library using Anim Layers in its source file, which could cause issues. Please clean its source file from anim layers or nla")
...
def turn_animlayers_on(self, context):
    ...
    if self.turn_on:
        ...
        if len(obj.Anim_Layers):
            ...
            #         bpy.context.window_manager.popup_menu(draw_override_error, title = "Message Box", icon = 'INFO')
    else:           
    ...
    for anim_data in anim_datas:
        ...
        if anim_data == selected_anim_data:
            ...
            if not len(obj.Anim_Layers) and len(anim_data.nla_tracks):
                ...
                continue
            if check_override_tracks(obj, anim_data):
...
def start_animlayers(obj):
    ...
    anim_data.action_extrapolation = 'HOLD'
    if not len(anim_data.nla_tracks):
        return
...
def remove_sub_track(anim_data):
    ...
    if not len(anim_data.nla_tracks[-1].strips):
        return
    #check if there are referenced nla tracks and if they include a subtrack
    ref_nlatracks = check_override_tracks(obj, anim_data)
    if ref_nlatracks:
        if 'Subtract_Layer' in ref_nlatracks[-1].name and ref_nlatracks[-1].strips[0].blend_type == 'SUBTRACT' and len(ref_nlatracks) == len(anim_data.nla_tracks):
            return        
...
def override_sub_track(obj, anim_data):
    override_tracks = check_override_tracks(obj, anim_data)
    if not override_tracks:
        return False
    if len(override_tracks) != len(anim_data.nla_tracks):
        return False
    if not len(override_tracks[-1].strips):
        return False
    if override_tracks[-1].strips[0].blend_type != 'SUBTRACT':
        return False
    if 'Subtract_Layer' in override_tracks[-1].name:
        return True
...
def update_sub_track(nla_tracks, obj):
    ...
    if len(nla_tracks[-1].strips) and len(nla_tracks) > 1:
        if nla_tracks[-1].strips[0].blend_type == 'SUBTRACT':
            if override_sub_track(obj, nla_tracks.data):
                nla_tracks[-1].mute = True
            else:
...
def update_layer_index(self, context):
    ...
    if current_action is not None:
        obj.als.fcurves = len(anim_data.action.fcurves)
    else:
        obj.als.fcurves = 0
...
def layer_name_update(self, context):
    ...
    if self.name != nla_tracks[index].name:
        ...
        if nla_tracks[index].name in override_tracks:
            override_tracks[nla_tracks[index].name].name = self.name
...
class OverrideError(bpy.types.Operator):
    ...
    def execute(self, context):
        if not self.confirm:
            return {'FINISHED'}
        filepath = context.object.override_library.reference.library.filepath
    ...
    def invoke(self, context, event):
        ...
        return wm.invoke_props_dialog(self, width=400+len(obj_name))
        ...
        for anim_data in anim_datas:
            ...
            if self.confirm:
                ...
                if check_override_tracks(obj, anim_data):
                    ...
                    continue
                # if override_tracks:
                #     bpy.context.window_manager.popup_menu(draw_error, title = 'override tracks', icon = 'ERROR')
                #     start_animlayers(obj)
            else:
                start_animlayers(obj)
...
class RemoveAnimLayer(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        anim_data = anim_data_type(obj)
        nla_tracks = anim_data.nla_tracks
        #nla_tracks_ref = list_override_nlatracks(obj)
        #if obj.als.layer_index < len(nla_tracks_ref):
...
classes = (ResetLayerKeyframes, LAYERS_UL_list, AddAnimLayer, ExtractSelection, ExtractMarkers, DuplicateAnimLayer, RemoveAnimLayer, CyclicFcurves, RemoveFcurves, MoveAnimLayerUp,
    MoveAnimLayerDown, SelectBonesInLayer, ANIMLAYERS_PT_List, ANIMLAYERS_PT_Ops, ANIMLAYERS_PT_Tools, ClearNLA, OverrideError)
