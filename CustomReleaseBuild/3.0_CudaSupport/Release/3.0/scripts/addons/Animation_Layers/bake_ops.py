...
def smart_bake(self, context):
    ...
    for track in anim_data.nla_tracks:
        ...
        for fcu in track.strips[0].action.fcurves:
            ...
            if len(fcu.modifiers) and obj.als.mergefcurves: #and obj.als.layer_index != i
...
def AL_bake(frame_start, frame_end, nla_tracks, fcu_keys, additive, baked_layer = None):
...
class MergeAnimLayerDown(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if obj.als.operator == 'MERGE':
            ...
            baked_layer = None
            action_name = anim_data.action.name        
        ...
        if obj.als.baketype =='BLENDER':
            ...
            action = AL_bake(frame_start, frame_end, nla_tracks, fcu_keys, additive, baked_layer)
