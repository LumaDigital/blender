...
class PL_OT_playblast(bpy.types.Operator):
    ...
    def playblast(self, context):
        ...
        for action in bpy.data.actions:
            if object.animation_data.action.name == action.name:
                try:
        ...
        for action in bpy.data.actions:
            if object.animation_data.action.name == action.name:
                try:
                    playblast_name = action.name
                    playblast_name = playblast_name.split('|')[1]
                except:
                    playblast_name = action.name
        ...
        # Create Camera and point at target rig
        #rig_object = bpy.data.objects[bpy.context.scene.vse_rig_name]
        #rig_location = rig_object.location
        #rig_rotation = rig_object.rotation_euler
        #location_offset = Vector(0.0, 0.0, 2.0)
        #location_offset = rig_object.location + location_offset
        #capture_camera = bpy.ops.object.camera_add(enter_editmode=False, align='VIEW', location=location_offset, rotation=rig_object.rotation_euler, scale=(1, 1, 1))

        #target_object = bpy.data.objects[bpy.context.scene.vse_rig_name]
        #location_offset = Vector((0.0, 2.0, 2.0)) + target_object.location
        #rotation_offset = Euler((1.2141592653, 0.0, 0.0))
        #bpy.ops.object.camera_add(enter_editmode=False, align='VIEW', location=location_offset, rotation=rotation_offset, scale=(1, 1, 1))
