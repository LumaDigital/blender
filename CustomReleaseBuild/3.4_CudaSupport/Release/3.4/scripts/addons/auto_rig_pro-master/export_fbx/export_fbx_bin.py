...
def fbx_data_material_elements(root, ma, scene_data):
    ...
    # Fix by Artell - Transparent color is exported as the base color, 
    # leading to Transparent instead of opaque materials in Unity depending on the diffuse color...
    # leave it to 1.0 instead (opaque)
    elem_props_template_set(tmpl, props, "p_color", b"TransparentColor", (1.0, 1.0, 1.0))#ma_wrap.base_color)
    ...
    for ob_obj in objects:
        ...
        if settings.use_mesh_modifiers or ob.type in BLENDER_OTHER_OBJECT_TYPES or is_ob_material:
            ...
            if not use_org_data:               
                ...
                tmp_me.name = ob_to_convert.data.name
...
def save_single(operator, scene, depsgraph, filepath="",
                ...
                export_action_only='all_actions',# Auto-Rig Pro implant
                **kwargs
                ):
