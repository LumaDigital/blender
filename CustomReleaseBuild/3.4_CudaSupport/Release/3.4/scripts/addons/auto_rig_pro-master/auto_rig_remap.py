...
class ARP_OT_VSE_Retarget_Mocap(bpy.types.Operator):
    """VSE Tool to automatically import, set up motion-capture (.bvh or .fbx) and ready the rigs for retargetting"""  
    
    bl_idname = "arp.remap_vse_automate_mocap_target"
    bl_label = "Mocap retarget"
    custom_key = "vse_selected_action"
    ...
    def invoke(self, context, event):
        ...
        for action in bpy.data.actions:
            if not action.name in self._actions_list:
                self._actions_list.append(action.name)
    ...
    def draw(self, context):
        ...
        for action_name in self._actions_list:
            ...
            self.draw_action_row(column, action_name)          
      
        layout.separator()      

    def draw_action_row(self, column, action_name):
        ...
        if len(action.keys()):
            if self.custom_key in action.keys():
                if action[self.custom_key]:
                    icon_name = 'CHECKBOX_HLT' # selected_action

        select_action_operator = row.operator('arp.fbx_action_selection', text='', icon=icon_name)
        select_action_operator.action_name = action_name
    ...
    def execute(self, context):
        ...
        if  self._is_bvh_file:
            ...
            for action in bpy.data.actions:
                if self.custom_key not in action.keys():
                    self.report({'ERROR'}, "No action selected!")
                    return {'CANCELLED'} 

                if action[self.custom_key]:
                    self._mocap_obj.animation_data.action = action
                    self.target_mocap()
                    break
...
class ARP_OT_VSE_Select_FBX_Action(Operator):
    """FBX animation selection"""
    bl_idname = "arp.fbx_action_selection"
    bl_label = "Select FBX animation"
    bl_options = {'UNDO'}
    ...
    def execute(self, context):
        for action in bpy.data.actions:
            if ARP_OT_VSE_Retarget_Mocap.custom_key not in action.keys():
                action[ARP_OT_VSE_Retarget_Mocap.custom_key] = None

            action[ARP_OT_VSE_Retarget_Mocap.custom_key] = action.name == self.action_name
...
class ARP_OT_remap_export_preset(Operator):
    ...
    def invoke(self, context, event):
        # get filepath
        custom_dir = bpy.context.preferences.addons[__package__].preferences.remap_presets_path
    ...
    def draw(self, context):
        ...
        if self.valid_directory == False:
            ...
            layout.prop(context.preferences.addons[__package__].preferences, "remap_presets_path", text="Custom Presets Path")
...
def draw_menu_freeze(self, layout):
    ...
    if not self.source_rig_is_frozen or not self.target_rig_is_frozen:
        ...
        if not self.source_rig_is_frozen:                
            layout.label(icon="INFO", text="Source armature rotation is not frozen: X, Y or Z values are different from 1")
...
class ARP_OT_bind_only(Operator):
    ...
    def invoke(self, context, event):
        ...
        if not self.unbind:# not relevant to freeze transforms when unbinding
            if self.invalid_arp_bones or ((not self.source_rig_is_frozen or not self.target_rig_is_frozen) and scn.arp_show_freeze_warn):
                return wm.invoke_props_dialog(self, width=450)
...
def _apply_offset(value, post_baking=False, set_selection=True):
...
def _save_pose_rest(self):
    ...
    # link back action
    source_rig.animation_data.action = bpy.data.actions.get(scn.source_action)
    #print("set source action", bpy.data.actions.get(scn.source_action))
    target_rig.data.pose_position = 'POSE'
...
def _retarget(self):
    ...
    if target_proxy_name:
        if get_object(scn.target_rig + "_local") == None:
            bpy.ops.object.duplicate(linked=False, mode='TRANSLATION')
            bpy.context.active_object.name = scn.target_rig + "_local"
        else:
            set_active_object(scn.target_rig + "_local")
    ...
    if is_already_bound == False and self.unbind == False:
        ...
        for bname in bone_names:
            ...
            #print("create remaptweak bone", eb.name+tweak_suffix)
            ...
            if bone_item.name != "" and bone_item.name != "None" and eb_source_bone and bone_item.name in tar_bones_dict:            
                ...
                if bone_item.set_as_root:
                    ...
                    if preserve:
                        ...
                        # apply mat diff of redefined rest pose
                        if eb_source_bone.name in mat_rest_diff:
                            root_pos.matrix = root_offset.matrix @ mat_rest_diff[eb_source_bone.name]                        
                        else:
                            print("Missing in mat_rest_diff", eb_source_bone.name)
                        """                  
                ...
                if bone_item.ik:
                    eb_source_bone = get_edit_bone(source_bone_name)
                    #remap_eb = get_edit_bone(bone_item.name+"_REMAP")
                    ...
                    if preserve:
                        ...
                        # apply mat diff of redefined rest pose                    
                        if eb_source_bone.name in mat_rest_diff:                            
                            ik_loc.matrix = ikloc_off.matrix @ mat_rest_diff[eb_source_bone.name]                        
                            print("Found in mat_rest_diff", eb_source_bone.name)
                        else:
                            print("Missing in mat_rest_diff", eb_source_bone.name)
                        """
                    ...
                    bpy.ops.object.mode_set(mode='POSE')   
                    '''
                    helper_bone = get_pose_bone(ikloc_name)                    
                    ...
                    if bone_item.ik_pole != "":
                        ...
                        if method == 1:
                            ...
                            # first try on Y
                            vec = source_rig.matrix_world.inverted() @ Vector((0.0, 0.01, 0.0))
                        ...
                        #print('track_bone_name', track_bone_name)
    ...
    if is_already_bound:
        if (self.bind_only and self.unbind) or (self.bind_only == False):
            ...
            removed_bones = []
            del_suffix = ['_LOC', '_LOC.001', '_NOROT', '_IKLOC', '_OFFSET', 'IKLOC_offset', '_ROOT']
            del_in = ['_REMAP', '_REMAPTWEAK']
            for ebone in source_rig.data.edit_bones: 
                del_ebone = False
                for suf in del_suffix:
                    if ebone.name.endswith(suf):
                        del_ebone = True
                        break
                
                for _str in del_in:
                    if _str in ebone.name:
                        del_ebone = True
                        break
...
def _export_config(filepath):
    ...
    for item in scn.bones_map:
        ...
        props_list = [item.name, str(item.location), str(item.ik_auto_pole), vec_to_string(item.rot_add), 
                        vec_to_string(item.loc_add), str(item.loc_mult), str(item.ik_create_constraints)]
...
def _import_config(self, custom_path = ""):
    ...
    for i in range(0, int(bone_counts)):
        ...
        if item_source_bone != "":
            preset_data[item_source_bone] = [item_target_bone, item_set_as_root, item_ik, item_ik_pole, item_location, item_ik_auto_pole, item_rot_add, item_loc_add, item_loc_mult, item_add_ik_cns]
...
def register():
    ...
    bpy.types.Scene.vse_bones_preset_path  = ( 
        bpy.props.StringProperty(
            ...
            subtype = "FILE_PATH",
            default = "N:\\Cricket\\assets\\Blender_Remap_Presets\\MoBu_2_Mixamo.bmap"))
