...
# Transfer weights with operators. Slower or faster than per vertex dependingo on the context
...
def transfer_weight_mod(object=None, dict=None, list=None, replace=False, tar_grp_name=""):
    ...
    for vgroup_name in vgroups_names_copy:
        vgroup = object.vertex_groups.get(vgroup_name)
        
        grp_name_base = get_bone_base_name(vgroup.name)
        side = get_bone_side(vgroup.name)
        ...
        if dict:
            if grp_name_base in dict:
                for tar_grp_base_name in dict[grp_name_base]:
                    ...
                    transfer_weight_mod_operator(object=object, src_grp_name=vgroup.name, tar_grp_name=tar_grp_name, replace=replace)
...
def transfer_weight_prefix_mod(object=None, prefix=None, tar_grp_base_name=None):
    ...
    for vgroup in object.vertex_groups:
        if vgroup.name.startswith(prefix):
            side = vgroup.name[-2:]
            tar_grp_name = tar_grp_base_name+side
            
            if object.vertex_groups.get(tar_grp_name):#if exists
                transfer_weight_mod_operator(object=object, src_grp_name=vgroup.name, tar_grp_name=tar_grp_name)
    ...
    for vgroup in object.vertex_groups:
        ...
        if dict:
            if grp_name_base in dict:
                ...
                if tar_grp:
                    clamp_weight_mod_operator(object=object, src_grp_name=vgroup.name, tar_grp_name=tar_grp_name)
...
def multiply_weight_mod(object=None, dict=None):  
    ...
    for vgroup in object.vertex_groups:    
        ...
        if grp_name_base in dict:
            ...
            multiply_weight_mod_operator(object=object, tar_grp_name=vgroup.name, fac=fac)         
